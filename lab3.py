...
def simpleTokenize(string):
    ...
    """
    return re.sub(split_regex,' ',string).lower().split()
...
def tokenize(string):
    ...
    """
    return [x for x in re.sub(split_regex,' ',string).lower().split() if x not in stopwords and x <> '' ]
...
def countTokens(vendorRDD):
    ...
    """
    #print vendorRDD.collect()
    new = vendorRDD.map(lambda(x,y): len(y))
    return sum(new.collect())
...
def findBiggestRecord(vendorRDD):
    ...
    new = (vendorRDD
           ...
           .map(lambda (x,y) : (x,y,len(y)))
           .map(lambda (x,y,z) : (z,(x,y)))
           .sortByKey(ascending=False)
           .map(lambda (z,(x,y)) : (x,y))
           )
    return new.collect()
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    return dict([(key,val*idfs[key]) for key, val in tfs.iteritems()])
...
trueDupsRDD = (sims
               .join(goldStandard))
#print "take 5",trueDupsRDD.take(5)
...
nonDupsRDD = (sims
              ...
              .filter(lambda (x,y) : y[1]==None)
             )
#print nonDupsRDD.take(100)

avgSimNon = sum(nonDupsRDD.map(lambda (x,y): y[0]).collect())/(nonDupsRDD.count())
#print avgSimNon
...
#print "tak 5",amazonWeightsRDD.take(5)
...
def swap(record):
    ...
    Returns:
        pair: ((ID, URL), token)
    """
    token = tokenize(record[0])
    keys = record[1]
    return (keys, token)

#print "rdd1",amazonInvPairsRDD.take(5)
#print "rdd2",googleInvPairsRDD.take(5)
...
def fastCosineSimilarity(record):
    ...
    s = 0
    for i in tokens:
        a = amazonWeightsBroadcast.value[amazonRec][i]
        b = googleWeightsBroadcast.value[googleRec][i]
        s += a*b
