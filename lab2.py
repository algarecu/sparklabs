...
#test = access_logs.map(lambda x: type(x.date_time)).take(1)
#print test
#hosts = access_logs.<FILL IN>

#uniqueHosts = hosts.filter(lambda x : x[1]==1)
...
dayToHostPairTuple = (access_logs
                      .map(lambda x : (x.date_time.day, x.host))
                      .distinct()
                      .map(lambda x : (x[0],1))
                      )
#print dayToHostPairTuple.take(3)
dayGroupedHosts = dayToHostPairTuple.reduceByKey(lambda x, y : x+y)
...
allByDay = (dayAndHostTuple
            .map(lambda x : (x[0],1))
            .reduceByKey(lambda x,y : x + y)
            )
#print allByDay.collect()
distinctByDay = (dayAndHostTuple
                 .distinct()
                 .map(lambda x: (x[0],1))
                 .reduceByKey(lambda x,y : x + y)
                 )
#print distinctByDay.collect()
avgDailyReqPerHost = (allByDay.join(distinctByDay)
...
badRecords = (access_logs
              .map(lambda log : (log.response_code,log.endpoint,log.host,log.date_time.day,log.date_time.hour))
...
badEndpoints = badRecords.map(lambda x : x[1])
...
badEndpointsCountPairTuple = badRecords.map(lambda x : (x[1],1))
...
errHostsCountPairTuple = badRecords.map(lambda x : (x[2],1))
...
errDateCountPairTuple = badRecords.map(lambda x : (x[3],1))
...
hourCountPairTuple = badRecords.map(lambda x: (x[4],1))
